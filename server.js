...
io.on("connection", (socket) => {
    ...
    socket.on("send-message", async ({ room = "general", text, sender, mediaUrl, mediaType }) => {
        if ((!text?.trim() && !mediaUrl) || !sender?.name) return;
        try {
            const msg = await Message.create({ room, text: text?.trim() || "", sender, mediaUrl, mediaType });
    ...
    socket.on("dm-send", async ({ from, to, text, mediaUrl, mediaType }) => {
        if ((!text?.trim() && !mediaUrl) || !from?.userId || !to?.userId) return;
        try {
